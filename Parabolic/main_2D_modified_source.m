...
for j = 1:length(t)
    ...
    source_M = (mu0)*((1-(B_M/M_max)).*((B_M)./(1 + (B_S/S_max)))); % source glioma cells
...
% In this section Videos are saved in Videos_source_modif folder
% To check if Videos_source_modif folder exists otherwise to create
if not(isfolder('Videos_source_modif'))
    mkdir('Videos_source_modif')
end
...
videofile = VideoWriter(strcat('Videos_source_modif/Tumor', video_ext),v_pro);
...
videofile = VideoWriter(strcat('Videos_source_modif/Acidity', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
...
%% This loop saves the results for each 30 days in folder Plots_source_modif in png
% or eps format with days in file names

% to check wheather Plots folder exists otherwise it makes a folder Plots
if not(isfolder('Plots_source_modif'))
    mkdir('Plots_source_modif')
end
...
for i = 1:10:size(M_total,3)
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_source_modif/Tumor_source_modif_%ddays',3*(i-1)),'epsc');%eps
    saveas(gcf,sprintf('Plots_source_modif/Tumor_source_modif_%ddays.png',3*(i-1)));%png
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_source_modif/Acidity_source_modif_%ddays',3*(i-1)),'epsc');
    saveas(gcf,sprintf('Plots_source_modif/Acidity_source_modif_%ddays.png',3*(i-1)))
end
...
xlabel('X' , 'Fontsize', 15);
ylabel('Y' , 'Fontsize', 15);
saveas(gcf,'Plots_source_modif/FA_source_modif.png');
%saveas(gcf,'Plots_source_modif/FA_source_modif','epsc');
...
xlabel('X' , 'Fontsize', 15);
ylabel('Y' , 'Fontsize', 15);
saveas(gcf,'Plots_source_modif/Q_macro_source_modif.png');
% saveas(gcf,'Plots_source_modif/Q_macro_source_modif','epsc');
...
xlabel('X' , 'Fontsize', 15);
ylabel('Y' , 'Fontsize', 15);
saveas(gcf,'Plots_source_modif/q_source_modif.png');
% saveas(gcf,'Plots_source_modif/q_source_modif','epsc');

%% uncomment to save the workspace
if not(isfolder('Mat_files'))
    mkdir('Mat_files')
end
save('Mat_files/main_2D_source_modif.mat');
%%
toc