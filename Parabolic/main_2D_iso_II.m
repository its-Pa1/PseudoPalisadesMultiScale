...
kappa = 1/3; % it controls the FA of D_w %
...
for j = 1:length(t)
    ...
    source_M = (mu0)*((1-(B_M/M_max)).*(1-(B_S/S_max))).*B_M; %glioma source term
...
% In this section Videos are saved in Videos_iso_II folder
% To check if Videos_iso_II folder exists otherwise to create
if not(isfolder('Videos_iso_II'))
    mkdir('Videos_iso_II')
end
...
videofile = VideoWriter(strcat('Videos_iso_II/Tumor', video_ext),v_pro);
...
videofile = VideoWriter(strcat('Videos_iso_II/Acidity', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
...
for i = 1:10:size(M_total,3)
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_iso_II/Tumor_iso_II_%ddays',3*(i-1)),'epsc');%eps
    saveas(gcf,sprintf('Plots_iso_II/Tumor_iso_II_%ddays.png',3*(i-1)));%png
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_iso_II/Acidity_iso_II_%ddays',3*(i-1)),'epsc');
    saveas(gcf,sprintf('Plots_iso_II/Acidity_iso_II_%ddays.png',3*(i-1)))
end
...
xlabel('X' , 'Fontsize', 15);
ylabel('Y' , 'Fontsize', 15);
saveas(gcf,'Plots_iso_II/FA_iso_II.png');
%saveas(gcf,'Plots_iso_II/FA_iso_II','epsc');
...
xlabel('X' , 'Fontsize', 15);
ylabel('Y' , 'Fontsize', 15);
saveas(gcf,'Plots_iso_II/Q_macro_iso_II.png');
% saveas(gcf,'Plots_iso_II/Q_macro_iso_II','epsc');
...
xlabel('X' , 'Fontsize', 15);
ylabel('Y' , 'Fontsize', 15);
saveas(gcf,'Plots_iso_II/q_iso_II.png');
% saveas(gcf,'Plots_iso_II/q_iso_II','epsc');

%% uncomment to save the workspace
if not(isfolder('Mat_files'))
    mkdir('Mat_files')
end
save('Mat_files/main_2D_iso_II.mat');
%%
toc