...
% In this case the diff. coeff. is a constant
tic
...
[D,A] = set_diff_mat_const(x,dt,DT_scale/(2*Ds_old), x_scale);
% [D,A] = set_diff_mat2(x,dt,2*DT_scale/(Ds_old), x_scale);
% [D,A] = set_diff_mat3(x,dt,DT_scale/(Ds_old), x_scale);
...
saveas(gcf,'Plots_patterns/Tumor_D_const.png');
...
axis tight
saveas(gcf,'Plots_patterns/Acid_D_const.png');

figure(3)
plot(x,D, 'Linewidth', 1.5)
xlabel('X' , 'Fontsize', 15);
ylabel('D_T' , 'Fontsize', 15);
axis([0 x(end) 0 2*max(D)]);
title('D_T', 'Fontsize', 15);
saveas(gcf,'Plots_patterns/Diff_D_const.png');
